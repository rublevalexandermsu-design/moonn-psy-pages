...
def build_program_pdf() -> None:
    ...
    doc = SimpleDocTemplate(
        ...
        topMargin=15 * mm,
        bottomMargin=16 * mm,
        title="Психология без скуки - программа подросткового интенсива речи и уверенности",
    )
    story = [
    ...
    story += [
        ...
        Paragraph("Для подростков, которым важно спокойнее общаться, легче знакомиться, увереннее говорить о себе, снять напряжение после экзаменов, пробовать нейросети для идей и понять первые шаги в профессии психолога.", body),
        Paragraph("Стандартная стоимость 50 000 ₽ отражает расширенный формат: личное ведение Татьяны Мунн, камерную группу, материалы и возможность экспертных блоков по речи, самопрезентации и проектным задачам.", body),
    ...
    schedule = [
        ...
        ("14:00-16:00", "Проектная работа: эмоции, уверенность, речь, ИИ, профессия или коммуникация."),
    ...
    days = [
        ...
        ("День 3", "Общение, границы и речь: практика диалогов, знакомства, просьб, мини-сцен и самопрезентации."),
...
def build_poster_pdf() -> None:
    ...
    canvas.drawString(26 * mm, page_h - 54 * mm, "речь, уверенность, общение и ИИ")
    ...
    bullets = [
        "общение, речь, уверенность, эмоции и личные границы;",
        "мягкая разгрузка после экзаменов и учебного напряжения;",
        "практика ИИ для идей, презентаций и мини-проектов;",
        "личное ведение и возможные экспертные блоки.",
    ]
    yy = 70 * mm
    for item in bullets:
...
def build_call_brief_docx() -> None:
    ...
    for heading, items in [
        ("Кому подходит", [
            ...
            "после ЕГЭ/ОГЭ хочется выдохнуть и мягко переключиться;",
            "важно легче знакомиться, говорить с разными людьми и выступать без зажима;",
        ...
        ("Что сказать в первые 30 секунд", [
            ...
            "Фокус: уверенность, общение, речь, эмоции, ИИ для идей, личный план и знакомство с профессией психолога.",
            "Татьяна Мунн ведет программу лично, группа 10-12 подростков.",
            "Стандартная стоимость 50 000 ₽ объясняется расширенным форматом: личное ведение, камерная группа, материалы и возможные экспертные блоки.",
        ]),
        ("Цена и оплата", [
    ...
    for time, text in [
        ...
        ("внутри дня", "короткие речевые практики: знакомство, самопрезентация, мини-выступление"),
