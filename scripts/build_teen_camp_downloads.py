...
def build_program_pdf() -> None:
    ...
    doc = SimpleDocTemplate(
        ...
        topMargin=15 * mm,
        bottomMargin=16 * mm,
        title="Психология без скуки - программа подросткового лагеря",
    )
    story = [
        Paragraph("Психология без скуки: подростковый лагерь", title),
        Paragraph("6-10 июля 2026 · Москва, Цветной бульвар · 10:00-18:00 · группа 10-12 подростков", body),
        Paragraph("Ранняя оплата до 15 июня: <b>40 000 ₽</b> вместо <strike>50 000 ₽</strike>. В программе есть перерывы и одноразовое питание.", body),
    ]

    cta_table = Table(
        [[
            LinkButton("Оплатить участие", PAYMENT_URL, 50 * mm, 12 * mm, colors.HexColor("#3149c9")),
            LinkButton("Страница лагеря", PUBLIC_PAGE_URL, 50 * mm, 12 * mm, colors.HexColor("#7b4be0")),
    ...
    schedule = [
        ...
        ("11:45-13:00", "Практика до обеда: упражнения, общение, мини-разборы."),
        ("13:00-14:00", "Обед и восстановление. Одноразовое питание включено в программу."),
    ...
    story += [
        Paragraph("Календарь следующих смен", h2),
        Paragraph("24-28 августа — «Разобраться в себе»; 19-20 и 26-27 сентября — «Уверенность в себе»; 26-30 октября — «Личная эффективность»; 2-6 ноября — «Выбор профессии».", body),
        Spacer(1, 5 * mm),
    ...
    def footer_canvas(canvas: Canvas, document) -> None:
        ...
        button(canvas, 78 * mm, 10 * mm, 54 * mm, 10 * mm, "Страница лагеря", PUBLIC_PAGE_URL, colors.HexColor("#7b4be0"))
        button(canvas, 136 * mm, 10 * mm, 54 * mm, 10 * mm, "Вопрос в Telegram", TELEGRAM_URL, colors.HexColor("#12a896"))
        canvas.setFont(FONT_REGULAR, 8)
        canvas.setFillColor(colors.HexColor("#6b7280"))
        canvas.drawRightString(w - 16 * mm, 6 * mm, "Moonn · подростковый лагерь · 2026")
        canvas.restoreState()
...
def build_poster_pdf() -> None:
    ...
    button(canvas, 73 * mm, 12 * mm, 53 * mm, 10 * mm, "Страница лагеря", PUBLIC_PAGE_URL, colors.HexColor("#7b4be0"))
...
def build_call_brief_docx() -> None:
    ...
    title_run = title_p.add_run("Памятка для созвона: подростковый лагерь «Психология без скуки»")
    title_run.bold = True
    ...
    lead.add_run("городской лагерь Татьяны Мунн для подростков: уверенность, общение, эмоции, ИИ для идей и первые шаги молодого психолога. 6-10 июля, Цветной бульвар, 10:00-18:00, группа 10-12 человек, одноразовое питание.")

    p = doc.add_paragraph()
    p.add_run("Главная страница лагеря: ").bold = True
    ...
    for heading, items in [
        ...
        ("Что сказать в первые 30 секунд", [
            "Это не школьный кружок и не терапевтическая группа, а камерный городской лагерь по психологии.",
        ...
        ("Цена и оплата", [
            ...
            "оплата открывается по ссылке выше на странице лагеря;",
    ...
    for time, text in [
        ...
        ("13:00-14:00", "обед, одноразовое питание, восстановление"),
    ...
    doc.add_heading("Календарь смен", level=1)
    for item in [
        "6-10 июля — базовая смена «Психология без скуки».",
    ...
    add_hyperlink(p, "Оплатить участие в лагере", PAYMENT_URL, bold=True)

    doc.add_heading("Частые вопросы", level=1)
    qa = [
        ("Нужно ли родителям присутствовать?", "Нет, родители не присутствуют на занятиях."),
        ("Есть ли питание?", "Да, предусмотрено одноразовое питание и перерывы."),
...
def replace_downloads() -> None:
    ...
    for name in old_names:
        ...
        if path.exists():
            path.unlink()
    shutil.copy2(PROGRAM_PDF, DOWNLOADS / "Программа подросткового лагеря Татьяны Мунн 2026.pdf")
    shutil.copy2(POSTER_PDF, DOWNLOADS / "Постер подросткового лагеря Татьяны Мунн 2026.pdf")
    shutil.copy2(CALL_BRIEF, DOWNLOADS / "Памятка для созвона по подростковому лагерю Татьяны Мунн 2026.docx")
