...
## 2026-06-20 — `мунн.рф` Daily Supervisor

1. `OK` Raw live-domain state stayed green: all 5 scoped `мунн.рф` URLs return HTTP `200`, remain in `sitemap.xml`, are not blocked by `robots.txt`, and keep the new-domain canonical. Evidence: `docs/moonn-five-page-seo-sprint-audit-2026-06-20.json`.
2. `OK` Rendered audit durability was improved: `scripts/moonn_five_page_seo_sprint_audit.py` now closes Playwright pages in `finally` and closes context/browser best-effort, so a retry produced dated artifacts after the first rendered attempt timed out. Evidence: `docs/moonn-five-page-seo-sprint-audit-2026-06-20-rendered.{json,md}`.
3. `P1` Rendered page-level gate is still mixed, not green: camp completed with `answerBlockCountRendered=1`, while gallery, exam prep, consultations and reviews still hit `Page.goto` `45000ms`.
4. `P0` GSC blocker was freshly re-proven in Chrome for the live resource `мунн.рф`: `У вас нет доступа к этому ресурсу`, so the canonical blocker text remains `новая property не подтверждена, traffic migration не доказан`.
5. `OK/P1` Yandex.Webmaster processed the five page URLs submitted on `2026-06-19 09:11 MSK`; `sitemap.xml` is still `В очереди`.
6. `P1` Source-level debt remains unchanged in raw HTML: camp raw H1 count `0`, consultations/reviews raw H1 count `2`, gallery placeholders `Your Name` / `Your Email`, and missing image `alt` across all 5 pages.
...
## 2026-06-22 — `мунн.рф` Daily Supervisor

1. `OK` Raw live-domain state stayed green: all 5 scoped `мунн.рф` URLs return HTTP `200`, remain in `sitemap.xml`, are not blocked by `robots.txt`, and keep the new-domain canonical. Evidence: `docs/moonn-five-page-seo-sprint-audit-2026-06-22.json`.
2. `OK/P1` Rendered audit completed and produced dated artifacts, but the page-level gate is still mixed: camp passes with rendered H1 `1` and answer block `1`; gallery, exam prep, consultations and reviews still hit `Page.goto` `45000ms`. Evidence: `docs/moonn-five-page-seo-sprint-audit-2026-06-22-rendered.json`.
3. `P0` Fresh GSC proof was not obtained today, so the latest confirmed 2026-06-20 proof still keeps blocker active: `новая property не подтверждена, traffic migration не доказан`.
4. `P1` Yandex.Webmaster status was not freshly rechecked today; latest confirmed 2026-06-21 proof still has the five submitted page URLs processed and `sitemap.xml` queued.
5. `P1` Source-level debt remains in raw HTML: camp raw H1 count `0`, consultations/reviews raw H1 count `2`, gallery placeholders `Your Name` / `Your Email`, and missing image `alt` across all 5 pages.
...
## 2026-06-23 — `мунн.рф` Daily Supervisor

1. `OK` Raw live-domain state stayed green: all 5 scoped `мунн.рф` URLs return HTTP `200`, remain in `sitemap.xml`, are not blocked by `robots.txt`, and keep the new-domain canonical. Evidence: `docs/moonn-five-page-seo-sprint-audit-2026-06-23.json`.
2. `OK/P1` Rendered audit completed and produced dated artifacts, but the page-level gate is still mixed: camp passes with rendered H1 `1` and answer block `1`; gallery, exam prep, consultations and reviews still hit `Page.goto` `45000ms`. Evidence: `docs/moonn-five-page-seo-sprint-audit-2026-06-23-rendered.json`.
3. `P0` Fresh GSC proof was not obtained today, so the latest confirmed 2026-06-20 proof still keeps blocker active: `новая property не подтверждена, traffic migration не доказан`.
4. `P1` Yandex.Webmaster status was not freshly rechecked today; latest confirmed 2026-06-21 proof still has the five submitted page URLs processed and `sitemap.xml` queued.
5. `P1` Source-level debt remains in raw HTML: camp raw H1 count `0`, consultations/reviews raw H1 count `2`, gallery placeholders `Your Name` / `Your Email`, and missing image `alt` across all 5 pages.
6. `P2` Process rule remains active: stale cabinet evidence can be carried forward only as latest-confirmed evidence, not as a fresh status update.
7. `P0` T+33 retirement decision remains premature: live Google property blocker is open/unconfirmed, rendered page-level gate is not green, and Yandex sitemap processing still needs a bounded re-check.
8. Next bounded action: safer GSC live-property check, Yandex sitemap status re-check, and split/harden rendered navigation for the 4 timeout pages without resubmitting processed URLs.

## 2026-06-26 — `мунн.рф` Daily Supervisor

1. `OK` Raw live-domain state stayed green after audit hardening: all 5 scoped URLs return HTTP `200`, remain in `sitemap.xml`, are not blocked by `robots.txt`, and keep live-domain canonical when Unicode `мунн.рф` and punycode `xn--l1acaw.xn--p1ai` are normalized before comparison. Evidence: `docs/moonn-five-page-seo-sprint-audit-2026-06-26.json`.
2. `DONE` Fixed a false-positive audit gate in `scripts/moonn_five_page_seo_sprint_audit.py`: sitemap/canonical comparisons now normalize IDN hostnames before comparing Unicode and punycode forms.
3. `P1` Rendered audit completed with a dated artifact, but the page-level gate remains mixed: camp passes with rendered H1 `1` and answer block `1`; gallery, exam prep, consultations and reviews still hit `Page.goto` `45000ms`.
4. `P0` Fresh GSC proof was not obtained today, so the latest confirmed GSC blocker remains active: `новая property не подтверждена, traffic migration не доказан`.
5. `P1` Yandex.Webmaster status was not freshly rechecked; latest confirmed proof still has the five submitted page URLs processed and `sitemap.xml` queued.
