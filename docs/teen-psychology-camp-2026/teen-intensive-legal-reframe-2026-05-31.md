...
- Draft page/package scan found no visible `лагерь`, `смена`, `10:00-18:00`, `один приём пищи`, `родители не присутствуют`, `12-19`, `оздоров`.
- Program PDF: 1 page, 6 clickable annotations, no `лагер`, no `10:00-18:00`, no old food claim.
- Poster PDF: 1 page, 3 clickable annotations, no `лагер`, no `10:00-18:00`, no old food claim.
- DOCX structural check: payment/page links present, no `лагер`, no `10:00-18:00`, no old food claim.
- DOCX visual render gate was not run because `soffice` is not available on this host.

## Publication Gate

Not published to Tilda in this draft step.

Before publishing:

1. Confirm final public name.
2. Confirm whether the business accepts the format change from `10:00-18:00` to `10:00-14:00`.
3. Confirm whether the old URL slug containing `lager` should be kept temporarily for SEO or migrated to a new alias such as `/podrostkovyy-intensiv-psihologiya` with redirect.
4. Confirm payment product text in Tilda cart should be renamed from `подростковый лагерь` to `подростковый интенсив`.
5. Run Tilda publish via authenticated Chrome and verify live HTML/rendered page/cart/downloads.
